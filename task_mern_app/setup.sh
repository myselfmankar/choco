...
# --- Proper MERN Assignment: VM Provisioning Script (Task App) ---
...
PROJECT_DIR=$(pwd)
NGINX_CONF="/etc/nginx/sites-available/mern_task"
...
server {
    ...
    # 2. Proxy API requests to Node backend (Port 5003)
    location /api/ {
...
cd $PROJECT_DIR
pm2 start backend/server.js --name "task-api"
pm2 save

echo "----------------------------------------------------"
echo "✅ SETUP COMPLETE! TASK MANAGER IS LIVE."
echo "----------------------------------------------------"
echo "🌎 Frontend: Serve by Nginx on Port 80"
echo "⚙️  Backend: Managed by PM2 (task-api)"
echo ""
echo "Manage your stack with these commands:"
echo " - View status: pm2 status"
echo " - View logs:   pm2 logs task-api"
echo " - Restart all: pm2 restart all"
echo " - DB Access:   mongosh task_db"
echo "----------------------------------------------------"
