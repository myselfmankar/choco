...
def setup_todo():
    ...
    with open("todo_nginx.conf", "w") as f:
        f.write(nginx_conf)
    
    run_it("sudo mv todo_nginx.conf /etc/nginx/sites-available/todo_app")
    run_it("sudo ln -sf /etc/nginx/sites-available/todo_app /etc/nginx/sites-enabled/")
    ...
    print(" To start the task server, run the following command:")
