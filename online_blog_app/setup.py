...
def setup_blog():
    ...
    with open("blog_nginx.conf", "w") as f:
        f.write(nginx_conf)
    
    run_it("sudo mv blog_nginx.conf /etc/nginx/sites-available/blog_app")
    run_it("sudo ln -sf /etc/nginx/sites-available/blog_app /etc/nginx/sites-enabled/")
    ...
    print(" To start the blog server, run the following command:")
