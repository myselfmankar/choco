...
# --- Proper MERN Assignment: VM Provisioning Script (E-Commerce) ---
...
PROJECT_DIR=$(pwd)
NGINX_CONF="/etc/nginx/sites-available/mern_ecommerce"
...
server {
    ...
    # 2. Proxy API requests to Node backend (Port 5002)
    location /api/ {
...
echo "Starting Ecommerce Store Services..."
cd $PROJECT_DIR
pm2 start backend/server.js --name "ecomm-api"
pm2 save
...
echo "⚙️  Backend: Managed by PM2 (ecomm-api)"
echo ""
echo "Manage your stack with these commands:"
echo " - View status: pm2 status"
echo " - View logs:   pm2 logs ecomm-api"
echo " - Restart all: pm2 restart all"
echo " - DB Access:   mongosh ecommerce_db"
echo "----------------------------------------------------"
